...
namespace EyesOfTheDragon.XRpgLibrary.TileEngine
{
    public class TileMap
    {
        ...
        public TileMap(List<Tileset> tilesets, List<MapLayer> layers)
        {
            ...
            for (int i = 1; i < layers.Count; i++)
            {
                if (mapWidth != mapLayers[i].Width || mapHeight != mapLayers[i].Height)
                    throw new Exception("Map layer size eception");
            }
        }
        ...
        public void Draw(SpriteBatch spriteBatch, Camera camera)
        {
            ...
            Point viewPoint = Engine.VectorToCell(
                    new Vector2((camera.Position.X + camera.ViewportRectangle.Width) * (1 / camera.Zoom),
